...
function PricingComponent({
    ...
    return (
        <div className="border border-gray-200 rounded-lg overflow-hidden transition-all hover:shadow-lg h-fit" ref={containerRef}>
            ...
            <div ref={featuresRef} className={`space-y-3 overflow-hidden transition-all duration-300 flex-grow ${!expanded && needsExpansion ? "max-h-[180px]" : "max-h-[1000px]"}`}>
                <ul className="space-y-3 mb-8">
                    {features.map((feature, index) => (
                        ...
                        <Check className="text-purple-600 w-5 h-5 mr-2 flex-shrink-0 mt-0.5" />
            ...
            {needsExpansion && (
                <button onClick={() => setExpanded(!expanded)} className="text-purple-600 font-medium flex items-center justify-center mb-6 hover:text-purple-800 transition-colors">
                    {expanded ? (
                    <>
            ...
            <Link
                to="/contact"
                className="block w-full bg-white border border-purple-600 text-purple-600 hover:bg-purple-50 text-center py-3 rounded-md font-medium transition-colors"
            >
                Get Started
            </Link>
            </div>
